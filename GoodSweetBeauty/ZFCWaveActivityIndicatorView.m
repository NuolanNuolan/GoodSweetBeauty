...
+(instancetype)show:(UIView *)view{
    ...
    ZFCWaveActivityIndicatorView *ZFCview = [[ZFCWaveActivityIndicatorView alloc]init];
    ZFCview.center = CGPointMake(SCREEN_WIDTH/2-3, ScreenHeight/2-64-5);
    
    [view addSubview:ZFCview];
    view.userInteractionEnabled =NO;
    [ZFCview startAnimating];
...
+(void)hid:(UIView *)view{
    ...
    ZFCWaveActivityIndicatorView *hud = [self HUDForView:view];
    if (hud != nil) {
...
+ (instancetype)HUDForView:(UIView *)view {
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            ...
            return (ZFCWaveActivityIndicatorView *)subview;
        }
    }
    return nil;
}
...
- (void)setupDefaults
{
    ...
    self.duration = 1.0f;
    self.delay = 0.1f;
    self.defaultColor = GETMAINCOLOR;
}

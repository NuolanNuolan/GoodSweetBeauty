...
- (void)configureSegmentButtonLayout {
    if([self.segmentButtons count] > 0) {
        ...
        //最下面底纹
        UIView *view_bot = [[UIView alloc]initWithFrame:CGRectMake(0, 43.5, SCREEN_WIDTH, 0.5)];
        view_bot.backgroundColor = RGB(221, 221, 221);
        [_segmentView addSubview:view_bot];
        ...
        if(CGSizeEqualToSize(self.segmentButtonSize, CGSizeZero)) {
            ...
            buttonHeight = self.segmentBarHeight-2;
        }else {
            buttonWidth = self.segmentButtonSize.width;
            buttonHeight = self.segmentButtonSize.height-2;
        ...
        for(int i = 0; i < [self.segmentButtons count]; i++) {
            ...
            if(i == 0) {
                ...
                segmentButton.enabled = NO;
                self.selectedButton = segmentButton;
                
                // 让按钮内部的label根据文字内容来计算尺寸
                [segmentButton.titleLabel sizeToFit];
                CGFloat indicatorViewW = segmentButton.titleLabel.frame.size.width;
                CGFloat indicatorViewCenterX = SCREEN_WIDTH/[self.segmentButtons count]/2;
                
                self.indicatorView.frame = CGRectMake(indicatorViewCenterX - indicatorViewW / 2, 42, indicatorViewW, 2);
            }
        ...
        [_segmentView addSubview:self.indicatorView];
    ...
    for(UIButton *b in self.segmentButtons) {
        if(b.tag - pagingButtonTag == buttonPage) {
            ...
            [UIView animateWithDuration:0.15 animations:^{
                CGFloat indicatorViewW = b.titleLabel.frame.size.width;
                CGFloat indicatorViewCenterX = b.center.x;
